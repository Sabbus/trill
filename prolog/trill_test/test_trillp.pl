...
test(p_wlbrcr_h, [blocked('Generate an infinite loop')]):-
  run((prob_instanceOf('WomanUnderLifetimeBRCRisk','Helen',Prob),close_to(Prob,0.123))).
test(ne_wlbrcr_h, [blocked('Generate an infinite loop')]):-
...
test(p_wa_wulbrcr, [blocked('Generate an infinite loop')]):-
  run((prob_sub_class('WomanAged3040','WomanUnderLifetimeBRCRisk',Prob),close_to(Prob,0.123))).
test(ne_wa_wulbrcr, [blocked('Generate an infinite loop')]):-
...
test(p_p_pp, [blocked('Generate an infinite loop')]):-
  run((prob_sub_class('dbpedia:Place','dbpedia:PopulatedPlace',Prob),close_to(Prob,0.8273765902816))).
test(ae_p_pp, [blocked('Generate an infinite loop')]):-
...
test(p_u_a, [blocked('Generate an infinite loop')]):-
