...
execute_query(M,QueryType,QueryArgsNC,Expl,QueryOptions):-
    ...
    ->
        (
            ...
            (
                query_option(QueryOptions,return_single_prob,false) 
            -> 
                true 
            ; 
                !
...
find_explanations(M,QueryType,QueryArgs,Expl,QueryOptions):-
    ...
    ( 
        query_option(QueryOptions,assert_abox,AssertABox) 
    -> 
        Opt=[assert_abox(AssertABox)] 
    ; 
        Opt=[]
    ),
    ( 
    ...
    ; 
        (
            (
                query_option(QueryOptions,return_prob,_) 
            -> 
                MonitorNExpl=all 
            ; 
                MonitorNExpl=bt
            ) 
        ) % if return_prob is present and no max_expl force to find all the explanations
    ),
    ( 
...
find_single_explanation(M,QueryType,QueryArgs,Expl,Opt):-
    ...
    ->  % TODO if QueryType is inconsistent no check
        (
            add_q(M,QueryType,Tableau,QueryArgs,Tableau0),
            set_up_tableau(M),
            findall(Tableau1,expand_queue(M,Tableau0,Tableau1),L),
            (
                query_option(Opt,assert_abox,true) 
            -> 
                (
                    writeln('Asserting ABox...'), 
                    M:assert(final_abox(L)), 
                    writeln('Done. Asserted in final_abox/1...')
                ) 
            ; 
                true
            ),
...
prune_tableau_rules(KBA,[min_rule|TR],[min_rule|PTR]):-
    (
        memberchk(minCardinality(_,_),KBA)
    ; 
        memberchk(minCardinality(_,_,_),KBA)
    ;
        memberchk(exactCardinality(_,_),KBA)
    ;
        memberchk(exactCardinality(_,_,_),KBA)
    ),
        !,
        prune_tableau_rules(KBA,TR,PTR).
...
prune_tableau_rules(KBA,[max_rule|TR],[max_rule|PTR]):-
    (
        memberchk(maxCardinality(_,_),KBA)
    ; 
        memberchk(maxCardinality(_,_,_),KBA)
    ;
        memberchk(exactCardinality(_,_),KBA)
    ;
        memberchk(exactCardinality(_,_,_),KBA)
...
:- use_module('./utility_translation.pl').
