:- module(tableau, [new_tableau/1, init_tableau/3, init_tableau/4, get_abox/2,
                   set_abox/3, get_tabs/2, set_tabs/3, get_clashes/2, 
                   set_clashes/3, absence_of_clashes/1, get_expansion_queue/2,
                   set_expansion_queue/3, extract_from_expansion_queue/3,
                   extract_from_expansion_queue_int/3, 
...
This module provides predicate to interact with tableaux.
...
/**
 * new_tableau(-EmptyTableau:Dict) is det.
 * 
 * Initialize an empty tableau as a dictionary with the following keys:
 * * `abox`: list of abox axioms
 * * `tabs`: TODO
 * * `clashes`: TODO
 * * `expq`: TODO
 */
...
/**
 * init_tableau(+ABox:abox, +Tabs:tableau, -InitializedTableaus:dict) is det.
 *
 * Initialize a tableau with the given ABox and Tabs.
 *
 * @param ABox TODO
 * @param Tabs TODO
 */
...
/**
 * init_tableau(+ABox:abox, +Tabs:tableau, +ExpansionQueue:expansion_queue, -InitializedTableaus:dict)
 * 
 * Initialize a tableau with the given ABox, Tabs and ExpansionQueue.
 *
 * @param ABox           TODO
 * @param Tabs           TODO
 * @param ExpansionQueue TODO
 */
init_tableau(ABox,Tabs,ExpansionQueue,tableau{abox:ABox, tabs:Tabs, clashes:[], expq:ExpansionQueue}).

/**
 * get_abox(+Tableau:Dict, -ABox:abox) is det.
 *
 * Returns the ABox list for the given Tableau.
 *
 * @param Tableau a tableau as dict
 */
get_abox(Tab,ABox):-
    ABox = Tab.abox.

/**
 * set_abox(+TableauIn:Dict, +NewABox:abox, -TableauOut:Dict) is det.
 *
 * Replace the `abox` of TableauIn with the given NewAbox.
 *
 * @param TableauIn a tableau as dict
 * @param NewABox   an abox as a list
 */
set_abox(Tab0,ABox,Tab):-
    Tab = Tab0.put(abox,ABox).

/**
 * get_tabs(+Tableau:Dict, -Tabs:tableau) is det.
 *
 * Returns Tabs for the given Tableau.
 *
 * @param Tableau a tableau as dict
 */
get_tabs(Tab,Tabs):-
...
/**
 * set_tabs(+TableauIn:Dict, +NewTabs:tableau, -TableauOut:Dict) is det.
 *
 * Replace the `tabs` of TableauIn with the given NewTabs.
 *
 * @param TableauIn a tableau as dict
...
/**
 * get_clashes(+Tableau:Dict, -Clashes:List) is det.
 *
 * Returns the Clashes list for the given Tableau.
 *
 * @param Tableau a tableau as dict
 */
...
/**
 * set_clashes(+TableauIn:Dict, +NewClashes:List, -TableauOut:Dict) is det.
 *
 * Replace the `clashes` of TableauIn with the given NewClashes.
 *
 * @param TableauIn  a tableau as dict
 * @param NewClashes a list of clashes
 */
set_clashes(Tab0,Clashes,Tab):-
    Tab = Tab0.put(clashes,Clashes).

/**
 * absence_of_clashes(+Tableau:Dict) is semidet.
 *
 * `true` if Tableau has an empty `clashes` list.
 *
 * @param Tableau  a tableau as dict
 */
...
/**
 * get_expansion_queue(+Tableau:Dict, -ExpansionQueue:expansion_queue) is det.
 *
 * Returns the ExpansionQueue for the given Tableau.
 *
 * @param Tableau a tableau as dict
 */
...
/**
 * set_expansion_queue(+TableauIn:Dict, +NewExpansionQueue:expansion_queue, -TableauOut:Dict) is det.
 *
 * Replace the `expq` of TableauIn with the given NewExpansionQueue.
 *
 * @param TableauIn         a tableau as dict
 * @param NewExpansionQueue an expansion queue
 */
...
merge_tabs(X,Y,(T0,RBN0,RBR0),(T,RBN,RBR)):-
    (
        neighbours(X,T0,LSX0)
    *->
        assign(LSX0,LSX);assign([],LSX)
    ),
    (
        neighbours(Y,T0,LSY0)
    *->
        assign(LSY0,LSY)
    ;   assign([],LSY)
    ),
